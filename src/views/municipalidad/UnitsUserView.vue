...
import RangeDateDialog from '@/modules/municipalidad/dialogs/RangeDateDialog.vue'
import UnitsUpdateDialog from '@/modules/municipalidad/dialogs/UnitsUpdateDialog.vue'
...
// ABRIR Y CERRAR MODAL DE AGREGAR
const openAddModal = () => {
...
const updateUnit = (updatedUnit) => {
  // Busca en tableData la unidad con el mismo idPropietario que la que estamos editando
  const index = tableData.value.findIndex(
    (u) => u.idPropietario === selectedUnit.value.idPropietario
  )
  if (index !== -1) {
    // Mezcla los nuevos campos con la fila original
    tableData.value[index] = { ...tableData.value[index], ...updatedUnit }
  }
  closeEditModal()
}

// ---- EJEMPLO DE DATOS "CONDUCTORES" (global)
// Si lo deseas, puedes armar tu lista de conductores de otra forma,
// O tomar todos los "conductor" que ya existan en tableData.
const allDrivers = ref([
  { licencia: 'ABC123', nombre: 'Juan Pérez' },
  { licencia: 'XYZ987', nombre: 'María López' },
...
<template>
  <section>
    ...
    <header>
      <h1
        class="text-colorText333 text-[28px] md:text-[32px] font-bold dark:text-white"
      >
        Gestión de Unidades
      </h1>
      <!-- Botones -->
      <div
        class="flex flex-col gap-2 lg:flex-row justify-between items-center mt-7"
      >
        <div
        ...
        >
          <button
            type="button"
            class="flex flex-1 lg:flex-auto items-center gap-2.5 px-4 py-3 lg:py-4 text-[14px] lg:text-[16px] font-medium text-gray-900 bg-white border border-gray-200 rounded-s-lg hover:bg-gray-100 hover:text-blue-700 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:text-white dark:hover:bg-gray-700"
          >
            <IconTableDown />
            Descargar excel
          </button>
    ...
    <!-- Contenido principal -->
    <main
      class="bg-white dark:bg-colorsecundary rounded-[20px] p-5 mt-7"
    >
      <!-- Filtros -->
      <div
        class="flex flex-col md:flex-row gap-2.5 md:gap-0 justify-between items-center"
      >
        <!-- Buscador -->
        ...
        <div
          class="flex border w-full lg:w-auto border-gray-300 dark:border-gray-600 rounded-lg"
        >
          <div
            class="hidden lg:flex p-3 border-r border-gray-300 dark:border-gray-600"
          >
            <IconFilter class="text-colorTextLight dark:text-white" />
          </div>
          <div
            class="border-r border-gray-300 dark:border-gray-600 w-full flex-1"
          >
            <select
              id="asociacion"
  ...
  <!-- MODAL DE EDICIÓN -->
  <!-- Observa que le pasamos 'selectedUnit' y 'allDrivers' -->
  <UnitsUpdateDialog
